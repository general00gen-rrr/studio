  ...
  return (
    <div className="pt-[88px] min-h-screen bg-white overflow-x-hidden">
      ...
      <div className="container-lux py-10 lg:py-16">
        <div className="grid md:grid-cols-2 gap-12 lg:gap-20 items-start min-w-0">
          ...
          <div className="md:sticky md:top-28 min-w-0 overflow-hidden">
            ...
            <ScrollReveal direction="up" delay={60}>
              <div className="flex items-center gap-3 mt-4">
                <div className="flex items-center gap-0.5">
                  {[...Array(5)].map((_, i) => (
                    <svg key={i} className={`w-3 h-3 ${i < Math.floor(product.rating || 0) ? 'text-gold' : 'text-lux-border'}`} fill="currentColor" viewBox="0 0 20 20">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                    </svg>
                  ))}
                </div>
                <span className="text-lux-gray text-xs">{product.rating || '—'} — {product.reviews || 0} avis vérifiés</span>
              </div>
            </ScrollReveal>
            ...
            <ScrollReveal direction="up" delay={160}>
              <div className="mt-7">
                <div className="flex border-b border-lux-border overflow-x-auto scrollbar-hide">
                  {(['details', 'livraison', 'avis'] as const).map(tab => (
                    <button key={tab} onClick={() => setActiveTab(tab)}
                      ...
                      {tab === 'details' ? 'Caractéristiques' : tab === 'livraison' ? 'Livraison' : 'Avis'}
                    </button>
                  ))}
                </div>
                <div className="py-5">
                  {activeTab === 'details' ? (
                    <div className="space-y-4">
                      {product.description && (
                        <p className="text-lux-gray text-sm leading-[1.8] font-light border-l-2 border-gold/30 pl-4" style={{whiteSpace:"pre-wrap",wordBreak:"break-all",overflowWrap:"anywhere"}}>{product.description}</p>
                      )}
                    ...
                    <div className="space-y-4">
                      {[
                        ...
                        <div key={i} className="border border-lux-border p-4 rounded-sm">
                          ...
                          <p className="text-[10px] tracking-[0.15em] text-lux-gray/50 mt-3 uppercase">{r.date}</p>
                        </div>
                      ))}
                    </div>
                  )}
