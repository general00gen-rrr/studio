            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1.5">الاسم الكامل<span className="text-sky-600 mr-1">*</span></label>
                    <input type="text" value={form.fullName} onChange={e => setForm({...form, fullName: e.target.value})} placeholder="مثال: محمد العلمي" className={`input-lux ${errors.fullName ? 'border-red-400' : ''}`} />
                    {errors.fullName && <p className="text-red-500 text-xs font-bold mt-1.5">{errors.fullName}</p>}
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1.5">رقم الهاتف (للتأكيد والتوصيل)<span className="text-sky-600 mr-1">*</span></label>
                    <input type="tel" value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} placeholder="06 12 34 56 78" className={`input-lux ${errors.phone ? 'border-red-400' : ''}`} />
                    {errors.phone && <p className="text-red-500 text-xs font-bold mt-1.5">{errors.phone}</p>}
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1.5">المدينة والعنوان الكامل<span className="text-sky-600 mr-1">*</span></label>
                    <input type="text" value={form.city} onChange={e => setForm({...form, city: e.target.value})} placeholder="مثال: أكادير، شارع الحسن الثاني، الدار البيضاء..." className={`input-lux ${errors.city ? 'border-red-400' : ''}`} />
            ...
            <div>
              <div className="bg-white border border-slate-200/80 rounded-2xl p-6 sticky top-24 shadow-sm">
                ...
                <div className="space-y-3 mb-4 max-h-64 overflow-y-auto">
                  {items.map(item => (
                    <div key={item.id} className="flex gap-3 items-center relative p-2 bg-slate-50 rounded-xl border border-slate-100">
                      ...
                      <button type="button" onClick={() => removeItem(item.id)} className="w-5 h-5 flex items-center justify-center text-slate-400 hover:text-red-500 font-bold">×</button>
                    </div>
                  ))}
                </div>
                ...
                <button type="submit" disabled={loading} className="btn-aqua w-full justify-center mt-5 text-sm py-3.5 rounded-xl disabled:opacity-60 shadow-lg shadow-sky-600/20">
                  <span>{loading ? 'جاري إرسال الطلب...' : `تأكيد الطلب — ${formatPrice(orderTotal)}`}</span>
                </button>
              </div>
            </div>
          </div>
